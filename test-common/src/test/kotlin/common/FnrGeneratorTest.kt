...
class FnrGeneratorTest {
    ...
    @Test
    fun `start må ligge innenfor sekvensen`() {
        shouldThrow<IllegalArgumentException> {
            FnrGenerator(start = -1L)
        }.message shouldBe "Start kan ikke være negativ."

        shouldThrow<IllegalArgumentException> {
            FnrGenerator(start = 19_999_900_001L)
        }.message shouldBe "Start kan ikke være større enn 19999900000."
    }
