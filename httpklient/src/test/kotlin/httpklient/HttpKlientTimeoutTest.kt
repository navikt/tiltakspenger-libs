...
/**
 * Verifiserer at både connect-timeout og per-request timeout fungerer korrekt — både når kallet
 * holder seg innenfor timeouten og når det overskrider den.
 *
 * Vi bruker `runBlocking` (ikke `runTest`) fordi vi måler reell veggklokketid: timeouten håndheves
 * av `java.net.http.HttpClient` i ekte tid, og `runTest` ville hoppet over `delay`-basert venting.
 *
 * Regresjonsvern: en kjent bug i ktor 3.5.0 gjorde at hvert kall ventet hele timeouten før det
 * returnerte — også når serveren svarte umiddelbart. Vi vil aldri se den oppførselen her, så de
 * positive testene sjekker eksplisitt at responsen kommer tilbake i *god tid før* timeouten utløper.
 */
internal class HttpKlientTimeoutTest {
    ...
    fun `rask respons innenfor timeout returnerer raskt og venter ikke til timeout utløper`() {
        runBlocking {
            withWireMock { wiremock ->
                ...
                // Skal komme tilbake langt raskere enn timeouten. Hvis kallet ventet hele timeouten
                // (slik ktor 3.5.0-bugen gjorde), ville dette vært ~5s.
    ...
    @Test
    fun `respons som overskrider per-request timeout gir Timeout uten a vente på serveren`() {
        runBlocking {
            withWireMock { wiremock ->
                wiremock.stubFor(
    ...
    fun `tilkobling som henger begrenses av connect-timeout, ikke av per-request timeout`() {
        runBlocking {
            ...
            // Et tilkoblingsforsøk dit henger til connect-timeouten fyrer (eventuelt feiler raskt med en
            // nettverksfeil på enkelte oppsett). Poenget: connect-fasen begrenses av den korte
            // connect-timeouten og henger IKKE til den langt romsligere per-request timeouten.
