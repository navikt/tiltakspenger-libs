...
import no.nav.tiltakspenger.libs.httpklient.infra.circuitbreaker.toCircuitBreaker
import no.nav.tiltakspenger.libs.httpklient.infra.retry.AttemptResult
import no.nav.tiltakspenger.libs.httpklient.infra.retry.RetryExecutor
 ...
 * En kastet exception blir en `Either.Left` med en [no.nav.tiltakspenger.libs.httpklient.infra.retry.AttemptOutcome.Failure]; en fullstendig HTTP-respons (uansett status) blir en `Either.Right`.
 ...
 * IO-dispatcher og rå bytes-lesing er transportens ansvar (se [no.nav.tiltakspenger.libs.httpklient.infra.transport.JavaHttpTransport]); dekoding skjer først i [tilTypetRespons] og metadata-byggingen i [finalize].
 */
