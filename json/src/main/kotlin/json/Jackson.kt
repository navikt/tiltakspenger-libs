...
/**
 * Utfører felles konfig på en ny [JsonMapper.Builder].
 * Tar ikke imot en eksisterende builder — det ville innbudt til mutasjon på tvers av kallsstedene (en delt builder kunne fått `banlistModule()` lagt på seg fra ett kall, og lekket inn i alle andre mappere).
 */
...
/**
 * Standard `ObjectMapper` for prosjektet — håndhever [BANLIST] for både serialisering og deserialisering, på *alle* nivåer (top-level, felt i DTO, element i List/Set, verdi i Map, nøkkel i Map, og vilkårlig dypt nestet).
 *
 ...
 *
 * Typer i [BANLIST] kaster på rot- og under-nivåer.
 * Sett [enforceBanlist] = false kun bevisst (testing/migrering) — det skrur av sjekken og åpner [objectMapperUtenBanlist].
 */
 ...
 *
 * Trigger ikke banlisten siden vi ikke binder til konkrete typer her — Jackson vet ikke at f.eks. `{"right":42}` "betyr" en bannlyst type.
 */
