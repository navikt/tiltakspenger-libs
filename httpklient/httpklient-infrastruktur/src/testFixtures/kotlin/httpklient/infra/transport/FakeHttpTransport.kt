package no.nav.tiltakspenger.libs.httpklient.infra.transport
 ...
 * Trådsikker: kø og opptak beskyttes av [lock], så faken kan deles mellom coroutines i en test (jf. at [no.nav.tiltakspenger.libs.httpklient.infra.HttpKlient] selv kan brukes parallelt).
 *
 * Simulér transportfeil med [leggIKøKast] og JDK-exceptions (`HttpTimeoutException` → [no.nav.tiltakspenger.libs.httpklient.HttpKlientError.Timeout], `IOException` → [no.nav.tiltakspenger.libs.httpklient.HttpKlientError.NetworkError]).
...
class FakeHttpTransport : HttpTransport {
    ...
    /** Køer en exception som kastes fra transporten — pipelinen mapper den til [no.nav.tiltakspenger.libs.httpklient.HttpKlientError.Timeout]/[no.nav.tiltakspenger.libs.httpklient.HttpKlientError.NetworkError]. */
