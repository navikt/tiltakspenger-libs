...
/**
 * Orkestrerer skip-cache-retryen beskrevet på [no.nav.tiltakspenger.libs.httpklient.infra.kall.AuthTokenProvider] og HttpKlientConfig.skipCacheRetryStatuses.
 *
 * [execute] kjøres først med `skipCache = false`.
 * Ett nytt forsøk med `skipCache = true` gjøres kun når _begge_ disse er sanne:
 *  - [providerStyrerAuthHeader] er `true` (dvs. en [no.nav.tiltakspenger.libs.httpklient.infra.kall.KlientAuth.System]-provider faktisk styrer `Authorization`-headeren), og
 *  - det første resultatet ble regnet som en _feil_ (`Left`) med en statuskode i [skipCacheRetryStatuses].
 *
 * At vi kun leser status fra en `Left` (og aldri fra en vellykket `Right`) er bevisst: en konsument som med vilje aksepterer f.eks. `401` som suksess via [no.nav.tiltakspenger.libs.httpklient.infra.kall.Statusregel.Eksakt] skal ikke få et uventet ekstra HTTP-kall.
 */
